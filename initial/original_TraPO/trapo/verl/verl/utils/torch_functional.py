...
# def certainty_from_logits(logits: torch.Tensor):
#     """Calculate certainty from logits."""
#     pd = torch.nn.functional.softmax(logits, dim=-1)
#     vocab_size = pd.shape[-1]

#     uniform_dist = torch.ones_like(pd) / vocab_size
#     self_certainty = F.kl_div(pd, uniform_dist, reduction='none').sum(dim=-1)

#     return self_certainty

def certainty_from_logits(logits):
    """Calculate certainty from logits."""
