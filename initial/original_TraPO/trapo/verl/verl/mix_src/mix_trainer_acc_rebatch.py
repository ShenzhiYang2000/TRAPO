...
class MIXRayPPOTrainerAccRebatch(MIXRayPPOTrainer):
    ...
    def _create_dataloader(self):
        ...
        if self.config.data.shuffle:
            ...
            unlabel_sampler = RandomSampler(data_source=self.unlabel_train_dataset, generator=train_dataloader_generator)
        else:
        ...
        self.unlabel_train_dataloader = BufferedDataLoader(DataLoader(dataset=self.unlabel_train_dataset,
        ...
        print(f'Size of unlabel train dataloader: {len(self.unlabel_train_dataloader)}')
    ...
    def fit(self):
        ...
        for _ in range(self.config.trainer.total_epochs):
            ...
            self.unlabel_train_dataloader.start_new_epoch() # NOTE: does this affect continue runnning?
            ...
            while True:  # batch iter
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.unlabel_train_dataloader.buffer_size() > 0:
                        unlabel_buffer_batch = self.unlabel_train_dataloader.get_from_buffer(unlabel_batch_size, self.actor_rollout_wg.world_size)

                    batch_size = self.config.data.train_batch_size
                    unlabel_batch_size = self.config.data.unlabel_train_batch_size
                    ...
                    while len(unlabel_valid_batch) < unlabel_batch_size * n_samples:  # construct a valid batch
                        try:
                            unlabel_batch_dict = self.unlabel_train_dataloader.get_next_batch()
                        except StopIteration:
                            break
                        
                        unlabel_batch: DataProto = DataProto.from_single_dict(unlabel_batch_dict)

                        if len(unlabel_buffer_batch) > 0:
                            try:
                                if 'prefix_ratios' in unlabel_buffer_batch.meta_info.keys():
                                    unlabel_buffer_batch.meta_info.pop('prefix_ratios')
                                if 'prefix_ratios' in unlabel_batch.meta_info.keys():
                                    unlabel_batch.meta_info.pop('prefix_ratios')
                                unlabel_batch = DataProto.concat([unlabel_buffer_batch, unlabel_batch])
                                unlabel_buffer_batch = []
                            except Exception as e:
                                breakpoint()
                        ...
                        # log avg prefix ratio
                        if 'prefix_ratios' in unlabel_gen_batch_output.meta_info.keys():
                        ...
                        with _timer('adv', timing_raw):
                            ...
                            if self.use_rm:
                                ...
                                unlabel_batch = unlabel_batch.union(unlabel_reward_tensor)
                        ...
                        if self.config.data.get('filter_accuracy', False):
                            unlabel_batch = self.filter(unlabel_batch.batch['token_level_scores'],unlabel_batch, self.config.actor_rollout_ref.rollout.n)
                        
                        # if len(buffer_batch) > 0:
                        #     batch = DataProto.concat([buffer_batch, batch])
                        #     buffer_batch = []

                        if len(unlabel_valid_batch) == 0:
                            unlabel_valid_batch = unlabel_batch
                        else:
                            unlabel_valid_batch = DataProto.concat([unlabel_valid_batch, unlabel_batch])
                    
                        print(
                            f"collected {len(unlabel_valid_batch)} / {unlabel_batch_size * n_samples} rollouts and each prompt has {n_samples} responses")
                    ...
                    if len(unlabel_valid_batch) < unlabel_batch_size * n_samples: 
                        break
                    elif len(unlabel_valid_batch) > unlabel_batch_size * n_samples:
                        unlabel_valid_batch = self.unlabel_add_to_buffer(unlabel_valid_batch, unlabel_batch_size, n_samples)
                        
                    batch = valid_batch
                    reward_tensor = batch.batch['token_level_scores']

                    unlabel_batch = unlabel_valid_batch
                    unlabel_reward_tensor = unlabel_batch.batch['token_level_scores']
                    ...
                    if self.config.actor_rollout_ref.actor.get('use_off_policy_loss', False) and self.config.actor_rollout_ref.actor.get('off_policy_loss_impl', 'seq') == 'seq':
                        ...
                        if self.config.actor_rollout_ref.actor.get('off_policy_normalize', True):
                            ...
                            unlabel_l = unlabel_prefix_mask.shape[-1]
                            assert unlabel_prefix_mask.shape[0] % n == 0
                            unlabel_bsz = unlabel_prefix_mask.shape[0] // n
                            unlabel_off_policy_mask = unlabel_prefix_mask.any(-1) # [bsz * n]
                            unlabel_on_policy_mask = ~unlabel_off_policy_mask # [bsz * n]
                            unlabel_logprobs = unlabel_batch.batch['old_log_probs'] # [bsz * n, l]
                            unlabel_op_logprobs = unlabel_logprobs[unlabel_on_policy_mask].reshape(unlabel_bsz, -1, unlabel_l) # check whether the shape is correct
                            unlabel_op_tok_logprobs = masked_mean(unlabel_op_logprobs, unlabel_op_logprobs!=0, axis=-1) # [bsz, n_on]
                            # op_seq_logprobs = op_logprobs.mean(dim=-1) # [bsz, n_on]
                            unlabel_op_logprobs_mean = unlabel_op_tok_logprobs.mean(dim=-1) # [bsz]
                            unlabel_op_logprobs_std = unlabel_op_tok_logprobs.std(dim=-1) # [bsz]
                            
                            unlabel_batch.batch['on_logprobs_mean'] = unlabel_op_logprobs_mean[:, None].repeat(1, n).reshape(-1) # [bsz]
                            unlabel_batch.batch['on_logprobs_std'] = unlabel_op_logprobs_std[:, None].repeat(1, n).reshape(-1) # [bsz]
                            ...
                            unlabel_bsz = unlabel_batch.batch['input_ids'].shape[0]
                            unlabel_dtype, unlabel_device = unlabel_batch.batch['advantages'].dtype, unlabel_batch.batch['advantages'].device
                            unlabel_batch.batch['on_logprobs_mean'] = torch.zeros(unlabel_bsz).to(unlabel_dtype).to(unlabel_device)
                            unlabel_batch.batch['on_logprobs_std'] = torch.zeros(unlabel_bsz).to(unlabel_dtype).to(unlabel_device)
                    ...
                    if self.config.actor_rollout_ref.actor.get('use_sft_prefix_reward', False):
                        ...
                        unlabel_batch.batch['advantages'][unlabel_prefix_mask] = reward_weight / self.config.actor_rollout_ref.rollout.n
    ...
    def unlabel_add_to_buffer(self, batch, batch_size, n_samples):
        buffer_length = len(batch) // n_samples - batch_size
        buffer_batch = batch.slice(range(batch_size * n_samples, (buffer_length + batch_size) * n_samples, n_samples))
        # buffer_batch.pop(non_tensor_batch_keys=['uid'], meta_info_keys=['prefix_ratios', 'global_token_num'])
        # notice that we only add prompts to buffer, and slicing strategy should be exactly consistent to what is in ray_trainer.py
        ...
        self.unlabel_train_dataloader.add_to_buffer(buffer_batch)
        return batch
