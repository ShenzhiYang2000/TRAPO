...
class MIXDataParallelPPOActor(DataParallelPPOActor):
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    # mean_off_log_prob = log_prob[off_policy_mask].mean(dim=0) # (1, V)
                    # on_log_prob = log_prob[on_policy_mask] # (b2, V)

                    # coff = mean_off_log_prob @ on_log_prob.T #(1, b2)
                    # print(coff.min())
                    # print(coff.max())
    ...
    def unlabel_update_policy(self, data: DataProto, use_ul_loss = True):
        # make sure we are in training mode
        self.actor_module.train()
        ...
        select_keys = ['responses', 'input_ids', 'attention_mask', 'position_ids', 'old_log_probs', 'advantages', 'prefix_mask', 'candidate_unlabel_mask']
        if self.config.use_kl_loss:
            select_keys.append('ref_log_prob')
        if self.config.use_off_policy_loss and self.config.off_policy_loss_impl == 'seq':
            select_keys.append('on_logprobs_mean')
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    candidate_unlabel_mask = data['candidate_unlabel_mask']
                    ...
                    if self.config.use_sft_multitask_loss:
                        ...
                        from .mix_core_alg import unlabel_compute_token_on_off_policy_loss
                        loss_fn = unlabel_compute_token_on_off_policy_loss
                        ...
                        ret_dict = loss_fn(old_log_prob=old_log_prob, 
                            ...
                            loss_remove_token_mean=self.config.loss_remove_token_mean,
                            loss_remove_clip=self.config.loss_remove_clip,
                            cadidate_mask = candidate_unlabel_mask
                        )
                        pg_loss = ret_dict['pg_loss']
                        off_pg_loss = ret_dict['off_pg_loss']
                        on_pg_loss = ret_dict['on_pg_loss']
                        ...
                        data = {
                            'ul_actor/off_pg_loss': off_pg_loss.detach().item(),
                            'ul_ctor/on_pg_loss': on_pg_loss.detach().item(),
                            'ul_actor/off_pg_clipfrac': off_pg_clipfrac.detach().item(),
                        }
                        if 'off_policy_prob' in ret_dict:
                            data['ul_actor/off_policy_prob'] = ret_dict['off_policy_prob'].detach().item()
                        if 'on_policy_prob' in ret_dict:
                            data['ul_actor/on_policy_prob'] = ret_dict['on_policy_prob'].detach().item()
                        if 'off_ratio_mean' in ret_dict:
                            data['ul_actor/off_ratio_mean'] = ret_dict['off_ratio_mean'].detach().item()
                        if 'off_ratio_max_clip_frac' in ret_dict:
                            data['ul_actor/off_ratio_max_clip_frac'] = ret_dict['off_ratio_max_clip_frac'].detach().item()
                        if 'off_ratio_min_clip_frac' in ret_dict:
                            data['ul_actor/off_ratio_min_clip_frac'] = ret_dict['off_ratio_min_clip_frac'].detach().item()
                        append_to_dict(metrics, data)
                    ...
                    if self.config.use_adaptive_temperature:
                        if self.config.use_adaptive_temperature_fixed is False:
                            ...
                            policy_loss = pg_loss - entropy_loss * entropy_coeff.detach().item()
                            metrics['ul_actor/alpha_loss'] = alpha_loss.detach().item()
                            metrics['ul_actor/entropy_coeff'] = entropy_coeff.detach().item()
                            metrics['ul_actor/log_alpha'] = self.log_alpha.detach().item()
                        else: # fixed strategy for entropy coeff
                            target_entropy = self.config.adaptive_temperature_target_entropy
                            # cur_entropy = verl_F.masked_mean(entropy, response_mask)
                            entropy_coeff = (target_entropy / entropy_loss).detach().item() * self.config.entropy_coeff
                            policy_loss = pg_loss - entropy_loss * entropy_coeff
                            metrics['ul_actor/entropy_coeff'] = entropy_coeff
                    else:
                        policy_loss = pg_loss - entropy_loss * entropy_coeff
                    ...
                    if self.config.use_kl_loss:
                        ...
                        policy_loss = policy_loss + kl_loss * self.config.kl_loss_coef
                        metrics['ul_actor/kl_loss'] = kl_loss.detach().item()
                        metrics['ul_actor/kl_coef'] = self.config.kl_loss_coef
                    if self.config.use_ppo_kl_loss:
                        policy_loss = policy_loss + ppo_kl.abs() * self.config.ppo_kl_loss_coef
                        metrics['ul_actor/ppo_kl_loss'] = ppo_kl.abs().detach().item()
                        
                    loss = policy_loss / self.gradient_accumulation

                    if not use_ul_loss or candidate_unlabel_mask is None:
                        loss *= 0.0

                    loss.backward()

                    data = {
                        'ul_actor/entropy_loss': entropy_loss.detach().item(),
                        'ul_actor/pg_loss': pg_loss.detach().item(),
                        'ul_actor/pg_clipfrac': pg_clipfrac.detach().item(),
                        'ul_actor/ppo_kl': ppo_kl.detach().item(),
                    }
                    append_to_dict(metrics, data)

                grad_norm = self._optimizer_step()
                data = {'ul_actor/grad_norm': grad_norm.detach().item()}
                append_to_dict(metrics, data)
        self.actor_optimizer.zero_grad()
        if self.alpha_optimizer is not None:
            self.alpha_optimizer.zero_grad()
