...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    """
    sequence_real_score = batch.batch["real_token_level_scores"].sum(-1)
    ...
    non_aborted_real_sequence_score = sequence_real_score[non_aborted_mask]
    non_aborted_sequence_score = sequence_score[non_aborted_mask]
    non_aborted_sequence_reward = sequence_reward[non_aborted_mask]

    real_score_mean = torch.mean(non_aborted_real_sequence_score).detach().item()
    real_score_max = torch.max(non_aborted_real_sequence_score).detach().item()
    real_score_min = torch.min(non_aborted_real_sequence_score).detach().item()
    ...
    metrics = {
        "critic/real_score/mean": real_score_mean,
        "critic/real_score/max": real_score_max,
        "critic/real_score/min": real_score_min,
        # score
...
def compute_data_metrics_semi(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    """
    Computes various metrics from a batch of data for PPO training.
