...
class RayPPOTrainer:
    ...
    def _generate_pseudo_labels_and_update_passrate_weak_noise_label(self, data: DataProto, epoch, metrics):
        ...
        for uid, indices in uid2idx.items():
            ...
            if epoch > self.config.trainer.start_select_epoch and self.config.trainer.use_trapo:
    ...
    # def certainty_from_logits(self, logits):
    #     """Calculate certainty from logits."""
    #     vocab_size = logits.shape[-1]
    #     log_probs = F.log_softmax(logits, dim=-1) + torch.log(torch.tensor(vocab_size, dtype=logits.dtype, device=logits.device))
    #     certainty = -torch.mean(log_probs, dim=-1)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # elif self.config.trainer.baseline == 'self_certainty':
                    #     old_self_certainty = self.actor_rollout_wg.compute_self_certainty(batch)
                    #     batch = batch.union(old_self_certainty)
